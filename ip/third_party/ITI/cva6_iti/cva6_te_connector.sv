...
module cva6_te_connector #(
   ...
   // parameter type iti_to_encoder_t = logic
    ...
    input logic [iti_pkg::NR_COMMIT_PORTS-1:0] valid_i,
    input iti_pkg::rvfi_to_iti_t rvfi_to_iti_i,

    output logic [iti_pkg::NR_COMMIT_PORTS-1:0] valid_o,
    output iti_pkg::iti_to_encoder_t iti_to_encoder_o
);
 ...
 // final $fclose(f);
  ...
  localparam type itt_out_t = struct packed {
    ...
    iti_pkg::itype_t         itype;
    logic                    ilastsize;
    logic [iti_pkg::XLEN-1:0] iaddr;
    riscv::priv_lvl_t        priv;
    logic [CAUSE_LEN-1:0]    cause;
    logic [iti_pkg::XLEN-1:0] tval;
    logic [63:0]             cycles;
  };

  logic interrupt;
  iti_pkg::itype_t [iti_pkg::NR_COMMIT_PORTS-1:0] itype;

  logic [IRETIRE_LEN-1:0] counter_d, counter_q;
  logic [iti_pkg::XLEN-1:0] addr_d, addr_q;
  logic special_d, special_q;

  uop_entry_t [iti_pkg::NR_COMMIT_PORTS-1:0] uop_entry;
  itt_out_t [iti_pkg::NR_COMMIT_PORTS-1:0] itt_out;

  logic [iti_pkg::NR_COMMIT_PORTS-1:0][IRETIRE_LEN-1:0] counter_itt;
  logic [iti_pkg::NR_COMMIT_PORTS-1:0][iti_pkg::XLEN-1:0] addr_itt;
  logic [iti_pkg::NR_COMMIT_PORTS-1:0] special_itt;
  logic [iti_pkg::NR_COMMIT_PORTS-1:0][CAUSE_LEN-1:0] cause_itt;
  logic [iti_pkg::NR_COMMIT_PORTS-1:0][iti_pkg::XLEN-1:0] tval_itt;

  logic [iti_pkg::NR_COMMIT_PORTS-1:0][IRETIRE_LEN-1:0] counter;
  logic [iti_pkg::NR_COMMIT_PORTS-1:0][iti_pkg::XLEN-1:0] addr;
  logic [iti_pkg::NR_COMMIT_PORTS-1:0] special;


  assign interrupt = rvfi_to_iti_i.cause[iti_pkg::XLEN-1];  // determined based on the MSB of cause

  for (genvar i = 0; i < iti_pkg::NR_COMMIT_PORTS; i++) begin
    itype_detector #(
        .ITYPE_LEN(ITYPE_LEN)
    ) i_itype_detector (
        .valid_i       (valid_i[i]),
  ...
  always_comb begin
    ...
    for (int i = 0; i < iti_pkg::NR_COMMIT_PORTS; i++) begin
  ...
  if (block_mode) begin
    assign counter_d = counter[iti_pkg::NR_COMMIT_PORTS-1];
    assign addr_d = addr[iti_pkg::NR_COMMIT_PORTS-1];
    assign special_d = special[iti_pkg::NR_COMMIT_PORTS-1];
  end
