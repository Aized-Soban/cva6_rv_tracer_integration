...
module tb_te_reg();
    ...
    always @(negedge clk) begin// on negedge we compare the expected result with the actual one
        // vector [0]
        $display("Test vector %0d", i);
        $display("Vector: %b", test_vector[i]);
        $display("Expected trace_enable: %b", expected_trace_enable);
        $display("Actual trace_enable: %b", trace_enable_o);
/*
        // upper_tvec
        ...
        // lower_tvec
        if (expected_lower_tvec !== lower_tvec_o) begin
            $display("Wrong lower_tvec: %b!=%b", expected_lower_tvec, lower_tvec_o);
        end
        // match_tvec
        if (expected_match_tvec !== match_tvec_o) begin
            $display("Wrong match_tvec: %b!=%b", expected_match_tvec, match_tvec_o);
        end
        // tvec_mode
        if (expected_tvec_mode !== tvec_mode_o) begin
            $display("Wrong tvec_mode: %b!=%b", expected_tvec_mode, tvec_mode_o);
        end
        // tvec_filter_o
        if (expected_tvec_filter !== tvec_filter_o) begin
            $display("Wrong tvec_filter: %b!=%b", expected_tvec_filter, tvec_filter_o);
        end
*/
        // lossless_trace
