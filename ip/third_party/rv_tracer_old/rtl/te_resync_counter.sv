...
module te_resync_counter #(
    parameter N = 2, // Number of packet emit signals (parallel packet lanes)
    ...
    input logic         trace_enabled_i, // it comes from filter // no it is from te_reg not filter
    input logic [N-1:0] packet_emitted_i, // Each bit means “a packet was emitted on this lane this cycle�?.
    ...
    */
    ...
    initial begin
    $monitor("resync_counter:%d",counter_d);
    $monitor("et_resync_max_o:%d",et_resync_max_o);
    $monitor("gt_resync_max_o:%d",gt_resync_max_o);
    //$monitor("resyn_clk:%d",clk_i);
