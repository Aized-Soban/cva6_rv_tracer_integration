...
initial begin
    /*$monitor("priority valid_i:%h at time:%h",valid_i,$time);
   // $monitor("priority valid_o:%h",valid_o);
   $monitor("priority packet out format:%h",packet_format_o);
   $monitor("priority packet out sync sub-format:%h",packet_f_sync_subformat_o);
   $monitor("tc_exc_only:%h,tc_ppccd:%h,tc_resync_br:%h,tc_er_n:%h,tc_rpt_br:%h,nc_exc_only:%h,nc_ppccd_br:%h,tc_f3_sf3:%h,reported_update:%h,tc_resync_o:%h",tc_exc_only,tc_ppccd,tc_resync_br,tc_er_n,tc_rpt_br,nc_exc_only,nc_ppccd_br,tc_f3_sf3,reported_update,tc_resync_o);
     $monitor(
    "t=%0t | tc_qualified_i=%0b tc_reported_q=%0b tc_first_qualified_i=%0b tc_ppccd=%0b tc_gt_max_resync_i=%0b !tc_resync_br_q=%0b lc_updiscon_i=%0b !tc_branch_map_empty_i=%0b cond_nc=%0b tc_rpt_br=%0b",
    $time,
    tc_qualified_i,
    tc_reported_q,
    ...
    (nc_exc_only || nc_ppccd_br || !nc_qualified_i),
    tc_rpt_br
  );*/
end    
