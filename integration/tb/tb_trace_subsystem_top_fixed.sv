...
module tb_trace_subsystem_top_fixed;
  ...
  task automatic wait_funnel_valid;
    ...
    begin
      ...
      captured_sel = funnel_mux_sel_o;
      $display("[OUT ] time=%0t sel=%0d id=0x%0h data=0x%08h bytes=%0d valid=%0b ready=%0b",
               $time, captured_sel, out_atid_o, out_atdata_o, out_atbytes_o,
...
begin
  if ((out_atid_o    === captured_id)   &&
    ...
    $display("[%s] FAIL: mismatch", tc_name);
    $display("        ENC: id=0x%0h data=0x%08h bytes=%0d",
             captured_id, captured_data, captured_bytes);
    $display("        OUT: id=0x%0h data=0x%08h bytes=%0d",
             out_atid_o, out_atdata_o, out_atbytes_o);
    $finish;
  end

  $display("[%s] INFO: mux select not checked at final output stage (post-mux FIFO decouples timing)", tc_name);
end
endtask
